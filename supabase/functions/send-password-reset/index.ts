// Supabase Edge Function — Şifre sıfırlama emailini Resend ile Rapor7/24 markalı gönderir
...
serve(async (req) => {
  ...
  try {
    ...
    // Admin client ile şifre sıfırlama linki üret
    ...
    if (!resetLink) throw new Error('Link üretilemedi')

    const html = `<!DOCTYPE html>
      <html><head><meta charset="utf-8" /></head><body>
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #3D4849; margin-bottom: 8px;">Rapor7/24 — Şifre Sıfırlama</h2>
        <p style="color: #555;">Merhaba,</p>
        <p style="color: #555;">
          Şifrenizi sıfırlamak için aşağıdaki butona tıklayınız.<br/>
        ...
        <p style="color:#aaa; font-size:12px; margin-top:32px;">
          ...
          Şifre sıfırlama talebinde bulunmadıysanız bu e-postayı yoksayınız.
        </p>
      </div></body></html>`
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: 'Rapor7/24 — Şifre Sıfırlama',
        html,
      }),
    })
