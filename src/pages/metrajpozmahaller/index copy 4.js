    ...
    <>
      ...
      {openLbsArray?.length > 0 &&
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: `auto 1fr auto ${metrajyapabilen_sutunlar} 0.5rem auto`, maxWidth: "70rem", gridTemplateAreas: gridAreasMahaller, mt: subHeaderHeight, pt: "1rem", pl: "1rem", pr: "1rem" }}>
          ...
          <>
            <Box sx={{ ...css_enUstBaslik, gridArea: "no", borderLeft: "1px solid black", justifyContent: "center" }}> {selectedPoz_metraj.pozNo} </Box>
            <Box sx={{ ...css_enUstBaslik, gridArea: "isim" }}> {selectedPoz_metraj.pozName}  </Box>
            <Box sx={{ ...css_enUstBaslik, gridArea: "onay", justifyContent: "center" }}> Miktar </Box>
            <Box sx={{ ...css_enUstBaslik, gridArea: "birimAd", justifyContent: "center" }}> Birim </Box>
          ...
          {/* HANGİ POZ İLE İŞLEM YAPILIYORSA - POZ İSMİ VE TOPLAM METRAJI */}
          <>
            <Box sx={{ ...css_enUstBaslik, gridArea: "top", justifyContent: "end", borderLeft: "1px solid black" }}> Toplam Metraj  </Box>
            <Box sx={{ ...css_enUstBaslik, gridArea: "miktar" }}>{ikiHane(selectedPoz_metraj?.onaylananMetraj)}</Box>
            <Box sx={{ ...css_enUstBaslik, gridArea: "birimBr", justifyContent: "center" }}>{selectedProje.pozBirimleri.find(x => x.id == selectedPoz_metraj?.pozBirimId)?.name}</Box>
            <Box sx={{ ...css_bosluk }}>.</Box>
            {
              selectedProje?.metrajYapabilenler.map((x, index) => {
                return (
