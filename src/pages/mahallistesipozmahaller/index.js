...
export default function P_MehalListesiPozMahaller() {
  ...
  const saveChange = async () => {
    ...
    try {
      ...
      const response = await fetch(process.env.REACT_APP_BASE_URL + `/api/dugumler`, {
        method: 'POST',
        headers: {
          email: appUser.email,
          token: appUser.token,
    ...
    <Box sx={{ m: "0rem", maxWidth: "60rem" }}>
      ...
      {!(isFetching1 || isFetching2) && openLbsArray?.length > 0 &&
        ...
        <Box sx={{ m: "1rem", mt: "4.5rem", display: "grid", gridTemplateColumns: gridTemplateColumns1 }}>
          ...
          <>
            <Box sx={{ ...css_enUstBaslik, borderLeft: "1px solid black", gridColumn: "1/3", justifyContent: "end" }}>
              Toplam Metraj
            </Box>
