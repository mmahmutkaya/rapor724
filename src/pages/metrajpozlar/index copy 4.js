...
export default function P_MetrajPozlar() {
  ...
  let metrajyapabilen_sutunlar = isProject?.metrajYapabilenler.reduce((acc, x, index) => index == 0 ? "auto" : acc + " auto", "")
  let users = isProject?.metrajYapabilenler.reduce((acc, x, index) => index == 0 ? "user" : acc + " user", "")
  let hazirs = isProject?.metrajYapabilenler.reduce((acc, x, index) => index == 0 ? "hazir" : acc + " hazir", "")
  console.log("hazirs",hazirs)
  ...
  const gridAreasMahaller = `
    'no isim . birimAd . ${hazirs} . onay'
    'no isim . birimAd . ${userSirali} . onay'
    'top top . birimBr . ${miktarSirali} . miktar'
  `
  ...
  return (
    ...
    <>
      ...
      {show == "MahalListesi_BirPoz" && selectedPoz && lbsArray?.length > 0 &&
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: `auto 1fr 0.5rem auto 0.5rem ${metrajyapabilen_sutunlar} 0.5rem auto`, maxWidth: "70rem", gridTemplateAreas: gridAreasMahaller, mt: subHeaderHeight, pt: "1rem", pl: "1rem", pr: "1rem" }}>
