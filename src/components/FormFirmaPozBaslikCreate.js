...
export default function FormPozBaslikCreate({ setShow }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const _projectId = firmaProject?._id
      ...
      if (typeof name === "string") {
        if (firmaProject.pozBasliklari.find(item => item.name == name)) {
      ...
      const result = await RealmApp?.currentUser?.callFunction("createPozBaslik", pozBilgi, firmaProject?._id);
      ...
      setFirmaProject(firmaProject => {
        let firmaProject_ = { ...firmaProject }
        firmaProject_.pozBasliklari.push(result)
  ...
  const handleChange_veriTuruId = (event) => {
    ...
    setveriTuruId(firmaProject.veriTurleri?.find(item => item.id === event.target.value).id);
  };


  const handleChange_haneSayisiId = (event) => {
    console.log("event.target.value", event.target.value)
    sethaneSayisiId(firmaProject.haneSayilari?.find(item => item.id === event.target.value).id);
  };
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
          ...
          <DialogContent>
            ...
            >
              ...
              >
                {
                  firmaProject?.veriTurleri?.map((oneVeriTuru, index) => (
            ...
            >
              ...
              >
                {
                  firmaProject?.haneSayilari?.map((oneHaneSayisi, index) => (
